...
export async function template1Step7(ctx: HearsContext<Context>) {
  try {
    const from = ctx.from;
    ...
    await editHtmlFileText(filePath, "p:eq(1)", heroText);

    const storedPrevMessage = prevMessage[from.id];
    const photo = new InputFile("./step-images/1/8.png");
    const caption = `"${heroText}" set as the second section's description.\n\nNext up, send the image for the second section's main image.\n\nStep 8 of 14`;
    const reply = await ctx.replyWithPhoto(photo, { caption });

    userState[from.id] = "1-8";
