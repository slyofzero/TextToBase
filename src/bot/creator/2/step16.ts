...
export async function template2Step16(ctx: HearsContext<Context>) {
  try {
    const from = ctx.from;
    ...
    await editHtmlFileText(filePath, "h2:eq(2)", heroText);

    const storedPrevMessage = prevMessage[from.id];
    const photo = new InputFile("./step-images/2/17.png");
    const caption = `"${heroText}" set as third section's title.\n\nNext up, send the third section's description in the next message.\n\nStep 17 of 22`;
    const reply = await ctx.replyWithPhoto(photo, { caption });

    userState[from.id] = "2-17";
