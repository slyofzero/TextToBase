...
export async function template2Step10(ctx: HearsContext<Context>) {
  try {
    const from = ctx.from;
    ...
    const photo = new InputFile("./step-images/2/11.png");
    const caption = `"${heroText}" set as the second feature's description.\n\nNext up, send the third feature in the next message.\n\nStep 11 of 22`;
    const reply = await ctx.replyWithPhoto(photo, { caption });

    userState[from.id] = "2-11";
