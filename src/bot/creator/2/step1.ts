...
export async function template2Step1(ctx: HearsContext<Context>) {
  try {
    const from = ctx.from;
    ...
    await editHtmlFileText(filePath, "h1:eq(1)", heroText);

    const storedPrevMessage = prevMessage[from.id];
    const photo = new InputFile("./step-images/2/2.png");
