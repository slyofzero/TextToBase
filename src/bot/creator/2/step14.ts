...
export async function template2Step14(ctx: HearsContext<Context>) {
  try {
    const from = ctx.from;
    ...
    await editHtmlFileText(filePath, "p:eq(5)", heroText);

    const storedPrevMessage = prevMessage[from.id];
    const photo = new InputFile("./step-images/2/15.png");
    const caption = `"${heroText}" set as the second section's description.\n\nNext up, send the link the section's button would lead to in the next message.\n\nStep 15 of 22`;
    const reply = await ctx.replyWithPhoto(photo, { caption });

    userState[from.id] = "2-15";
