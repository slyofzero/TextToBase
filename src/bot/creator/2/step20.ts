...
export async function template2Step20(ctx: HearsContext<Context>) {
  try {
    const from = ctx.from;
    ...
    await editHtmlFileText(filePath, "h3:eq(3)", heroText);

    const storedPrevMessage = prevMessage[from.id];
    const photo = new InputFile("./step-images/2/21.png");
    const caption = `"${heroText}" set as footer section's title.\n\nNext up, send the footer section's description in the next message.\n\nStep 21 of 22`;
    const reply = await ctx.replyWithPhoto(photo, { caption });

    userState[from.id] = "2-21";
