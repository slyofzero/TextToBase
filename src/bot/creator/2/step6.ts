...
export async function template2Step6(ctx: HearsContext<Context>) {
  try {
    const from = ctx.from;
    ...
    await editHtmlFileText(filePath, "h2:eq(0)", heroText);

    const storedPrevMessage = prevMessage[from.id];
    const photo = new InputFile("./step-images/2/7.png");
    const caption = `"${heroText}" set as section title.\n\nNext up, send the first feature of your website in the next message.\n\nStep 7 of 22`;
    const reply = await ctx.replyWithPhoto(photo, { caption });

    userState[from.id] = "2-7";
