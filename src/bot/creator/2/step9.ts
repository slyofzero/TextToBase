...
export async function template2Step9(ctx: HearsContext<Context>) {
  try {
    const from = ctx.from;
    ...
    const photo = new InputFile("./step-images/2/10.png");
    const caption = `"${heroText}" set as the second feature.\n\nNext up, send the second feature's description in the next message.\n\nStep 10 of 22`;
    const reply = await ctx.replyWithPhoto(photo, { caption });

    userState[from.id] = "2-10";
