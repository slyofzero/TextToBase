...
export async function template2Step18(ctx: HearsContext<Context>) {
  try {
    const from = ctx.from;
    ...
    await editHtmlLink(filePath, "a:eq(3)", navLink);

    const storedPrevMessage = prevMessage[from.id];
    const photo = new InputFile("./step-images/2/19.png");
    const caption = `"${navLink}" set as link to the button.\n\nNext up, send the fourth section's title.\n\nStep 19 of 22`;
    const reply = await ctx.replyWithPhoto(photo, { caption });

    userState[from.id] = "2-19";
