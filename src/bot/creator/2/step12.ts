...
export async function template2Step12(ctx: HearsContext<Context>) {
  try {
    const from = ctx.from;
    ...
    await editHtmlFileText(filePath, "p:eq(4)", heroText);

    const storedPrevMessage = prevMessage[from.id];
    const photo = new InputFile("./step-images/2/13.png");
    const caption = `"${heroText}" set as the third feature's description.\n\nNext up, send the second section's title in the next message.\n\nStep 13 of 22`;
    const reply = await ctx.replyWithPhoto(photo, { caption });

    userState[from.id] = "2-13";
