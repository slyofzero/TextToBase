...
export async function template2Step15(ctx: HearsContext<Context>) {
  try {
    const from = ctx.from;
    ...
    await editHtmlLink(filePath, "a:eq(2)", navLink);

    const storedPrevMessage = prevMessage[from.id];
    const photo = new InputFile("./step-images/2/16.png");
    const caption = `"${navLink}" set as link to the button.\n\nNext up, send the third section's title.\n\nStep 16 of 22`;
    const reply = await ctx.replyWithPhoto(photo, { caption });

    userState[from.id] = "2-16";
