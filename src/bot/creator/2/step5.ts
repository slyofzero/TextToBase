...
export async function template2Step5(ctx: HearsContext<Context>) {
  try {
    const from = ctx.from;
    ...
    await editHtmlFileText(filePath, "a:eq(0)", heroText);

    const storedPrevMessage = prevMessage[from.id];
    const photo = new InputFile("./step-images/2/6.png");
    const caption = `"${heroText}" set as title text for the website.\n\nNext up, send the section title for the next section.\n\nStep 6 of 22`;
    const reply = await ctx.replyWithPhoto(photo, { caption });

    userState[from.id] = "2-6";
